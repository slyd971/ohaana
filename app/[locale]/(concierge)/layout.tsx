...
import {
  LayoutDashboard, MessageSquare, Users2,
  CreditCard, Settings, LogOut, Menu,
} from 'lucide-react'

const NAV = [
  { href: '/concierge/dashboard', label: 'Tableau de bord', Icon: LayoutDashboard },
  { href: '/concierge/requests',  label: 'Demandes',         Icon: MessageSquare },
  { href: '/concierge/clients',   label: 'Clients',          Icon: Users2 },
  { href: '/concierge/commissions', label: 'Commissions',    Icon: CreditCard },
  { href: '/concierge/settings',  label: 'Paramètres',       Icon: Settings },
]
...
export default function ConciergeLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-dvh flex bg-sand">
      <aside
        className={cn(
          'fixed inset-y-0 left-0 z-40 w-60 bg-[#1A2E3D] flex flex-col transition-transform duration-300',
      ...
      >
        <div className="px-5 py-5 border-b border-coconut/10">
          ...
          <p className="text-coconut/40 text-[11px] mt-1 uppercase tracking-widest">Espace concierge</p>
        </div>
